...
public class Sample {
    private static final String DB_URL = "jdbc:mysql://127.0.0.1:3306/remerakgl";
    private static final String DB_USER = "root";
    private static final String DB_PASSWORD = "raymond1";
    ...
    private static void processEncounters() {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            String query = "SELECT e.client_id, e.encount_id FROM encounters e WHERE e.rhie_status = 2 ORDER BY e.time ASC";
            try (PreparedStatement stmt = conn.prepareStatement(query);
                 ResultSet rs = stmt.executeQuery()) {
                Map<Integer, List<Integer>> encounters = new LinkedHashMap<>();
                while (rs.next()) {
                    encounters.computeIfAbsent(rs.getInt("client_id"), k -> new ArrayList<>()).add(rs.getInt("encount_id"));
                }
    ...
    private static void executeScriptsForClient(Connection conn, int clientID, List<Integer> encounterIDs) {
        try {
            PreparedStatement stmt = conn.prepareStatement("UPDATE encounters SET rhie_status = 3 WHERE client_id = ? AND rhie_status = 2");
    ...
    private static void executePhpScript(String script, int clientID, int encounterID) {
        try {
            String phpCommand = "php " + script + " " + clientID + " " + encounterID;
            Process process = Runtime.getRuntime().exec(phpCommand);
            process.waitFor();
            System.out.println("Executed: " + script + " for Client ID: " + clientID + ", Encounter ID: " + encounterID);
        } catch (IOException | InterruptedException e) {
            e.printStackTrace();
        }
    }

    private static void markEncounterAsCompleted(Connection conn, int encounterID) {
        try {
            PreparedStatement stmt = conn.prepareStatement("UPDATE encounters SET rhie_status = 1 WHERE encount_id = ?");
            stmt.setInt(1, encounterID);
            stmt.executeUpdate();
            System.out.println("Encounter ID " + encounterID + " marked as completed.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
